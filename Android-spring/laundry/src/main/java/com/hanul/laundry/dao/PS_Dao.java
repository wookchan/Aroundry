...
//	Search에서 뿌릴 데이터 가져오기 : **전체 리스트**
	public ArrayList<StoreDTO> anSearch(){
		...
		try {
			...
			String query = "select s.*, nvl(machine - co, machine) rest_cnt "
					+ " from store s left outer join "
					+ "				(select count(*) co, storeid "
			...
			while (resultSet.next()) {
				...
				String rest_cnt = resultSet.getString("rest_cnt");
				dtos.add(new StoreDTO(storeid, location, imageurl, address, operating, f_cctv,
						f_game, f_toilet, f_concent, f_wifi, f_coin, ownerid, latitude, longitude, cost, rest_cnt));						
			}	
...
//	BookMark Search에서 뿌릴 데이터 가져오기 : **전체 리스트**
	public ArrayList<StoreDTO> anSearch3(String userid){
		// 데이터베이스와 연동하여 원하는 결과물을 얻는다.
		ArrayList<StoreDTO> dtos = new ArrayList<StoreDTO>();
		...
		try {
			...
			String query = "select s.*, nvl(machine - co, machine) rest_cnt "
					...
					+ "               group by storeid) ss "
					+ " on s.storeid = ss.storeid "
					+ " where s.storeid in (select storeid from bookmark where userid= '" + userid +  "') ";
